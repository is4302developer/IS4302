...
contract EndorsementToken is ERC721Full {
  ...
  mapping(address => Ownership) ownership; //owner => tokenID
  ...
  function createTokens (address citizen) public {
    ...
    require(ownership[citizen].isExists == false); //each citizen can only endorse once, modify this code if need to increase
    ownership[citizen].tokenId = _id;
    ownership[citizen].isExists = true;
  ...
  function getTokenId(address citizen) public view returns (uint256) {
      return ownership[citizen].tokenId;
  }
